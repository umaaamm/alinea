...
<div class="container-fluid">
						...
						foreach ($dataListResume as $key) {
							...
							<tbody>
								<tr>
									...
									<td><?php echo $key['judul_resume'];?></td>
									...
									<td>
										...
										<button class="btn btn-primary" type="button" data-original-title="Edit Resume" data-toggle="modal" data-target="#editModal" title="" onclick="edit('<?php echo $key['id_resume_nasional'];?>','<?php echo $key['judul_resume'];?>','<?php echo $key['id_makalah'];?>','<?php echo $key['id_unit_kerja'];?>','<?php echo $key['id_area'];?>','<?php echo $key['id_cabang'];?>','<?php echo $key['id_kategori'];?>','<?php echo $key['nama_file'];?>')"><i class="fa fa-pencil"></i></button></td>							
									</tr>
									<?php $a++; } ?>
								</tbody>
							</table>
	...
	<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<form method="post" enctype="multipart/form-data" action="<?php echo base_url();?>Karyawan/EditResumeKaryawan">
					<div class="modal-header">
						<h5 class="modal-title">Edit Data Resume</h5>
						<button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
					</div>
					<div class="modal-body">
						<input type="hidden" name="id_resume_nasional" id="id" readonly>
						<div class="form-group">
							<label for="validationCustom01">Judul Resume</label>
							<input class="form-control" id="jdl_mkl" type="text" placeholder="Judul Resume" name="judul_resume" required="Judul Resume Tidak Boleh Kosong">
						</div>
						<div class="form-group">
							<label for="exampleFormControlSelect9">Judul Makalah PIA</label>
							<select class="form-control digits" name="id_makalah" id="idmkl">
								<option value="-">- Pilih Salah Satu -</option>
								<?php foreach($listMakalah as $row_k):?>
									<option value="<?php echo $row_k->id_makalah;?>"><?php echo $row_k->judul_makalah;?></option>
								<?php endforeach;?>
						...
						<div class="row">
							<label class="col-lg-12 control-label">Upload File</label>
							<label id="nmpresume" class="col-lg-12 control-label"></label>
							<div class="col-md-4 mb-3">
	...
	<script type="text/javascript">
		...
		function edit(id,jdl_mkl,id_makalah,unitKerja,areaID,cabang,kategoriID,nama_file_makalah){
			$('#id').val(id);
			$('#jdl_mkl').val(jdl_mkl);
			$('#idmkl').val(id_makalah)
			$('#areaID').val(areaID);
			$('#cabang').val(cabang);
			$('#kategoriID').val(kategoriID);
			$('#nmpresume').text(nama_file_makalah);
